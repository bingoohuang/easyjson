module github.com/bingoohuang/easyjson/benchmark

go 1.12

require (
	github.com/bingoohuang/easyjson v0.0.0-00010101000000-000000000000
	github.com/json-iterator/go v1.1.7
	github.com/pquerna/ffjson v0.0.0-20190813045741-dac163c6c0a9
	github.com/ugorji/go/codec v1.1.7
	github.com/ugorji/go/codec/codecgen v1.1.7
	golang.org/x/tools v0.0.0-20190829051458-42f498d34c4d // indirect
)

replace github.com/bingoohuang/easyjson => ../
